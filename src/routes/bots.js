...
router.post("/add", async (req, res) => {
  try {
    ...
    if (!req.body.name) {
      return res
        .status(200)
        .json({ message: "Bot name is required", status: 400 });
    }

    if (!req.body.token) {
      return res
        .status(200)
        .json({ message: "Bot token is required", status: 400 });
    }
...
router.put("/update/:id", getTokenMiddleware, async (req, res) => {
  try {
    const newBot = new BotChat({ ...req.body });
    const saveBot = await newBot.save();
...
router.put("/add-group", async (req, res) => {
  try {
    if (!req.body.botId) {
      return res
        .status(200)
        .json({ message: "Bot Id is required", status: 400 });
    }

    if (!req.body.groupId) {
      return res
        .status(200)
        .json({ message: "Group Id is required", status: 400 });
    }

    if (!req.body.name) {
      return res
        .status(200)
        .json({ message: "Group name is required", status: 400 });
    }
    ...
    const checkExistId = bot.listGroup.filter(
      (group) => group.groupId == req.body.groupId
    );

    const checkExistName = bot.listGroup.filter(
    ...
    if (checkExistId.length > 0) {
      return res
        .status(200)
        .json({ message: "Group already exists", status: 400 });
    }

    if (checkExistName.length > 0) {
      return res
        .status(200)
        .json({ message: "Group name already exists", status: 400 });
    }
    ...
    const updateBot = await BotChat.findOneAndUpdate(
      ...
      {
        $push: {
          listGroup: { name: req.body.name, groupId: req.body.groupId },
        },
      },
      { new: true }
    );
...
router.put("/remove-group", async (req, res) => {
  try {
    ...
    const checkExist = bot.listGroup.filter(
      (group) => group.groupId === req.body.groupId
    );
    ...
    const update = await BotChat.findOneAndUpdate(
      { _id: req.body.botId },
      {
        listGroup: bot.listGroup.filter(
          (group) => group.groupId != req.body.groupId
        ),
      },
      { new: true }
